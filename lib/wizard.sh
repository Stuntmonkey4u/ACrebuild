...
run_setup_wizard() {
    welcome_message
    print_message "$BLUE" "--- ACrebuild Setup Wizard ---" true
    ...
    while true; do
        ...
        read -r -p "Enter path or press ENTER for default: " ac_dir
        ...
        if [ -d "$ac_dir" ]; then
            ...
            read -r -p "It might be a new clone location. (y/n): " confirm_path
            if [[ "$confirm_path" =~ ^[Yy]([Ee][Ss])?$ ]]; then
                break
            fi
        fi
    done
    ...
    if [ -f "$AZEROTHCORE_DIR/docker-compose.yml" ]; then
        ...
        read -r -p "Enter choice [y]: " docker_choice
        if [[ -z "$docker_choice" || "$docker_choice" =~ ^[Yy]([Ee][Ss])?$ ]]; then
            USE_DOCKER=true
        else
            USE_DOCKER=false
        fi
    ...
    if [ "$USE_DOCKER" = true ]; then
        ...
        read -r -p "Enter user or press ENTER for default: " db_user_input
    ...
    read -r -p "Enter path or press ENTER for default: " backup_dir_input
    ...
    CMAKE_C_COMPILER="$DEFAULT_CMAKE_C_COMPILER"
    CMAKE_CXX_COMPILER="$DEFAULT_CMAKE_CXX_COMPILER"
    CMAKE_BUILD_FLAGS="$DEFAULT_CMAKE_BUILD_FLAGS"
