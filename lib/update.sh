...
update_source_code() {
    ...
    if [ ! -d "$AZEROTHCORE_DIR" ]; then
        handle_error "AzerothCore directory not found at $AZEROTHCORE_DIR. Cannot update."
    fi

    local current_branch
    current_branch=$(git -C "$AZEROTHCORE_DIR" rev-parse --abbrev-ref HEAD 2>/dev/null)

    if [ -z "$current_branch" ]; then
        handle_error "Could not determine the current git branch in $AZEROTHCORE_DIR."
    fi
...
self_update_script() {
    ...
    sleep 2 # Give user a moment to read -r the message
        ...
        for sql_file in "${sql_files_to_apply[@]}"; do
            ...
            if [ -n "$db_name" ]; then
                ...
                if is_docker_setup; then
                    (cd "$AZEROTHCORE_DIR" && cat "$sql_file" | "$DOCKER_EXEC_PATH" compose exec -i -T -e MYSQL_PWD="$(get_secure_db_pass)" ac-database mysql -u"$DB_USER" "$db_name")
                else
                    cat "$sql_file" | MYSQL_PWD="$(get_secure_db_pass)" mysql -u"$DB_USER" "$db_name"
                fi
                if [ $? -eq 0 ]; then
...
update_modules() {
    ...
    while true; do
        ...
        read -r -p "$(echo -e "${YELLOW}${BOLD}Enter choice [1-3]: ${NC}")" choice
        echo ""
        ...
        if [[ "$choice" == "3" ]]; then
            ...
            while true; do
                ...
                echo ""
                print_message "$BLUE" "-----------------------------------------------" true
                read -r -p "$(echo -e "${YELLOW}${BOLD}Enter module number to update or $back_option_number to go back: ${NC}")" specific_choice
