...
restore_backup() {
    ...
    (
        ...
        for DB_NAME in "${DATABASES[@]}"; do
            ...
            if ! grep -q -iE "CREATE TABLE|INSERT INTO" "$SQL_FILE"; then
                print_message "$RED" "Error: Valid SQL content not found in $DB_NAME.sql. Backup archive is likely corrupted." true
                rm -rf "$TEMP_RESTORE_DIR"
                return 1
            fi
